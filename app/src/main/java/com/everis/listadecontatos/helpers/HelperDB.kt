package com.everis.listadecontatos.helpers

import android.content.ContentValues
import android.content.Context
import android.database.sqlite.SQLiteDatabase
import android.database.sqlite.SQLiteOpenHelper
import com.everis.listadecontatos.feature.listacontatos.model.ContatosVO

class HelperDB(
    context: Context
) : SQLiteOpenHelper(context, NOME_BANCO, null, VERSAO_ATUAL) {

    companion object {
        private val NOME_BANCO = "contato.db"
        private val VERSAO_ATUAL = 2
    }

    val TABLE_NAME = "contato"
    val COLUMNS_ID = "id"
    val COLUMNS_NOME = "nome"
    val COLUMNS_TELEFONE = "telefone"
    val DROP_TABLE = "DROP TABLE IF EXISTS $TABLE_NAME"
    val CREATE_TABLE =
            "CREATE TABLE $TABLE_NAME (" +
            "$COLUMNS_ID INTEGER NOT NULL," +
            "$COLUMNS_NOME TEXT NOT NULL," +
            "$COLUMNS_TELEFONE TEXT NOT NULL," +
            "" +
            "PRIMARY KEY($COLUMNS_ID AUTOINCREMENT)" +
            ")"

    override fun onCreate(db: SQLiteDatabase?) {
       db?.execSQL(CREATE_TABLE)
    }

    override fun onUpgrade(db: SQLiteDatabase?, oldVersion: Int, newVersion: Int) {
        if(oldVersion != newVersion) {
            db?.execSQL(DROP_TABLE)
        }
         onCreate(db)
    }

    fun buscarContatos(busca: String, isBuscaPorID: Boolean= false) : List<ContatosVO> {
        val db = readableDatabase ?: return mutableListOf()
        var lista = mutableListOf<ContatosVO>()

        var where: String? = null
        var args: Array<String> = arrayOf()

        if (isBuscaPorID) {
            where = "$COLUMNS_ID = ?"
            args = arrayOf("$busca")
        } else {
            // Modifiquei para buscar o nome e o telefone.
            where = "$COLUMNS_NOME LIKE ? OR $COLUMNS_TELEFONE LIKE ?"
            args = arrayOf("%$busca%", "%$busca%")
        }

        /*
         1ª forma de busca
         val sql = "SELECT * FROM $TABLE_NAME WHERE $COLUMNS_NOME LIKE ?"
         var buscaComPercentual = "%$busca%"
         var cursor = db.rawQuery(sql, arrayOf(buscaComPercentual))
         */

        /*
        2ª forma de busca (usando o SQL para fazer a busca)
        var where = "$COLUMNS_NOME LIKE ?"
        var buscaComPercentual: Array <String> = arrayOf("%$busca%")
        var cursor = db.query(TABLE_NAME, null, where, buscaComPercentual, null, null, null)
        */

        /* Modifiquei para buscar o nome e o telefone.
        val sql = "SELECT * FROM $TABLE_NAME WHERE $COLUMNS_NOME LIKE ? OR $COLUMNS_TELEFONE LIKE ?"
        var cursor = db.rawQuery(sql, arrayOf("%$busca%", "%$busca%"))
        */

        var cursor = db.query(TABLE_NAME, null, where, args, null, null, null)
        if (cursor == null) {
            db.close()
            return mutableListOf()
        }
        while (cursor.moveToNext()) {
            var contato = ContatosVO(
            cursor.getInt(cursor.getColumnIndex(COLUMNS_ID)),
            cursor.getString(cursor.getColumnIndex(COLUMNS_NOME)),
            cursor.getString(cursor.getColumnIndex(COLUMNS_TELEFONE))
            )
                lista.add(contato)
        }
        db.close()
        return lista
    }

    fun salvarContato(contato: ContatosVO) {
        val db = writableDatabase ?: return
        /*
        1ª forma de fazer insert (não recomendada)
        val sql = "INSERT INTO $TABLE_NAME ($COLUMNS_NOME,$COLUMNS_TELEFONE) VALUES ('${contato.nome}','${contato.telefone}')"
        db.execSQL(sql)
        db.close()
         */

        /*
        2ª forma de fazer o insert
        val sql = "INSERT INTO $TABLE_NAME ($COLUMNS_NOME,$COLUMNS_TELEFONE) VALUES (?,?)"
        var array = arrayOf(contato.nome, contato.telefone)
        db.execSQL(sql, array)
        db.close()
         */

        // 3ª forma de fazer o insert
        var content = ContentValues()
        content.put(COLUMNS_NOME, contato.nome)
        content.put(COLUMNS_TELEFONE, contato.telefone)
        db.insert(TABLE_NAME, null, content)
        db.close()
    }

    fun deletarContato(id: Int) {
        val db = writableDatabase ?: return

    /* 1ª forma
        val where = "id = ?"
        val arg = arrayOf("$id")
        db.delete(TABLE_NAME, where, arg)
        db.close()
     */

        // 2ª forma
        val sql = "DELETE FROM $TABLE_NAME WHERE $COLUMNS_ID = ?"
        val arg = arrayOf("$id")
        db.execSQL(sql,arg)
        db.close()
    }

    fun updateContato(contato: ContatosVO) {
        val db = writableDatabase ?: return

    /*  1ª forma
        val content = ContentValues()
        content.put(COLUMNS_NOME, contato.nome)
        content.put(COLUMNS_TELEFONE, contato.telefone)
        val where = "id = ?"
        val arg = arrayOf("${contato.id}")
        db.update(TABLE_NAME, content, where, arg)
        db.close()
     */

        // 2ª forma
        val sql = "UPDATE $TABLE_NAME SET $COLUMNS_NOME = ?, $COLUMNS_TELEFONE = ? WHERE $COLUMNS_ID = ?"
        val arg = arrayOf(contato.nome, contato.telefone, contato.id)
        db.execSQL(sql,arg)
        db.close()
    }


}